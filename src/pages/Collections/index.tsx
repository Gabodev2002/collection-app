...
export const Collections = () => {
    ...
    const [ hasMore, setHasMore ] = useState(true)
                ...
                useEffect(() => {
                    ...
                    try {
                        const res = await fetch(`https://api.unsplash.com/topics?page=${page}&per_page=8&client_id=Q2RreKXKTluL3jgHrsWvM97jT_iu8kpJmxkY-u8h_T0`)
                        if(!res.ok) {
                        throw new Error()
                        }
    ...
    return(
        <Layout>
            ...
            <section className='w-full h-auto bg-[#ffffff] pt-40 '> 
            ...
            <div className='px-4 grid grid-cols-1 md:grid-cols-2 md:max-w-[820px] lg:max-w-[1350px] lg:grid-cols-4 justify-items-center gap-5 md:px-8 mb-20  m-auto'>
                {collections?.map((collection) => (
                    ...
                    <article className={`${imgLoaded ? 'opacity-100 transition ease-in duration-500' : 'opacity-0'} relative overflow-hidden w-full max-w-[350px] h-60 cursor-pointer img-container rounded-sm`} key={collection.id}>
                            <NavLink to={`/collections/${collection.title}/${collection.id}}`}>
